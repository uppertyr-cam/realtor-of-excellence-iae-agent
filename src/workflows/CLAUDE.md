...
- Adding a new keyword: add the action to the `ROUTE_LEAD_TOOL` enum in `src/ai/generate.ts`, add the case to `handleKeyword()` in `ai-send-router.ts`, add the type to `DetectedKeyword` in `src/utils/types.ts`, and update `prompts/conversation.txt`
