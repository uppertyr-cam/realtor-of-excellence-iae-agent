...
import { processDripQueue } from '../workflows/workflow-00'
...
function extractLastAIMessage(aiMemory: string | null): string {
  ...
  // Walk backwards to find the last line starting with "AI:" or "AI (ai_reply):"
  for (let i = lines.length - 1; i >= 0; i--) {
    ...
    if (line.startsWith('AI:') || line.startsWith('AI (ai_reply):')) {
      ...
      // Truncate to ~120 chars to fit WA template variable limit
...
async function processBumpJob(job: any) {
  ...
  // New: nested template arrays [bump_group][variation_index]
  ...
  // If no templates configured, cancel and skip
  ...
  // If all bump groups exhausted, cancel
  if (bumpIndex >= bumpTemplateGroups.length) {
    await db.query(`UPDATE outbound_queue SET status='cancelled' WHERE id=$1`, [job.id])
    return
  }

  // Get template from [bump_group][variation]
  ...
  // Extract last AI message from conversation history for context
  const lastAIMsg = extractLastAIMessage(contact.ai_memory)
  const aiContext = lastAIMsg || 'some questions I had for you'

  // Message substitution: {{first_name}} and {{last_message}} (for SMS/fallback)
  const message = template
    .replace(/{{first_name}}/g, contact.first_name || '')
    .replace(/{{last_message}}/g, aiContext)

  // WA template names indexed by [bump_group][variation]
  ...
  let result
  if (channel === 'whatsapp' && waTemplateName) {
    // For WA templates: variables are {{1}}=first_name, {{2}}=ai_context
    result = await sendWhatsAppTemplate(
      contact.phone_number,
      waTemplateName,
  ...
  // Increment bump_index so the next bump uses the next template group
...
async function processBumpCloseQueue() {
  const res = await db.query(
    `UPDATE outbound_queue SET status='processing'
     WHERE id IN (
       SELECT id FROM outbound_queue
...
async function processBumpCloseJob(job: any) {
  ...
  // Tag contact as no reply after bumps
  await db.query(
    `UPDATE contacts SET tags=array_append(tags,'bump_no_reply') WHERE id=$1`,
    [contact.id]
  )

  // Write CRM note with full conversation summary
  await writeToCrm(
    {
      contact_id: contact.id,
  ...
  // Update live dashboard — contact is now in "Never Responded"
